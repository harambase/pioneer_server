...
public interface TranscriptApi {
    ...
    @ApiOperation(value = "更新成绩单", notes = "权限：管理员，教务", response = Map.class, tags = {Tags.PERSON})
    @ApiResponses(value = {@ApiResponse(code = 200, message = "操作成功", response = Map.class)})
    ResponseEntity update(@ApiParam(value = "成绩单", required = true) Transcript transcript, HttpSession session);

    @ApiOperation(value = "成绩单列表", notes = "权限：管理员，教务，教师，学生", response = Map.class, tags = {Tags.PERSON})
