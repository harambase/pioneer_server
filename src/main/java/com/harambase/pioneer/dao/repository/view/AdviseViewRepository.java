...
public interface AdviseViewRepository extends JpaRepository<AdviseView, Integer> {
    ...
    @Query("select a from AdviseView a where studentid = ?5 and (studentid like concat('%', ?1, '%') or facultyid like concat('%', ?2, '%') or sname like concat('%', ?3, '%') or fname like concat('%', ?4, '%'))")
