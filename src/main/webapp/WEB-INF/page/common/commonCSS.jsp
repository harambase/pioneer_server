<%--
  Created by IntelliJ IDEA.
  User: sky
  Date: 2017/8/7
  Time: 14:23
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@include file="commonInner.jsp"%>
<link rel="stylesheet" href="${basePath}/static/plugins/bootstrap-3.3.5-dist/css/bootstrap.css">
<link rel="stylesheet" href="${basePath}/static/plugins/font-awesome-4.7.0/css/font-awesome.css">
<link rel="stylesheet" href="${basePath}/static/plugins/datatables/datatables.min.css">
<link rel="stylesheet" href="${basePath}/static/plugins/datatables/plugins/bootstrap/datatables.bootstrap.css">
<link rel="stylesheet" href="${basePath}/static/plugins/jedate/skin/jedate.css">
<link rel="stylesheet" href="${basePath}/static/plugins/confirm/confirm.css">
