...
(/** @lends <global> */function( $ ) {
	 ...
	 *    the paging is reset to the first admin
	 *  @memberof DataTable#oApi
	 */
	...
	/**
	 * Add the options to the admin HTML for the table
	 *  @param {object} oSettings dataTables settings object
	 *  @memberof DataTable#oApi
	 */
	 ...
	 *  @param {array} json.aaData The data to display on this admin
	...
	/**
	 * Alter the display settings to change the admin
	 *  @param {object} settings DataTables settings object
	 *  @param {string|int} action Paging action to take: "first", "previous",
	 *    "next" or "last" or admin number to jump to (integer)
	 *  @param [bool] redraw Automatically draw the update or not
	 *  @returns {bool} true admin has changed, false - no change
	 *  @memberof DataTable#oApi
	 */
	...
	DataTable = function( options )
	{
		 ...
		 *  @param {string} [oOpts.admin=all] Limit the selection to the currently displayed admin
		 ...
		 * on display on the admin, with the HTML contents that is passed into the
		 ...
		 *    or admin number to jump to (integer), note that admin 0 is the first admin.
		 *  @param {bool} [bRedraw=true] Redraw the table or not
		 *  @dtopt API
		 *  @deprecated Since v1.10
		 *
		...
		this.each(function() {
			...
			{
				/* Grab the data from the admin - only do this when deferred loading or no Ajax
	 ...
	 * table, all tables on a admin or a sub-set thereof.
	 *
	 ...
	 *
	 * @return {integer} Current admin index (zero based)
	 *//**
	 * Set the current admin.
	 *
	 * Note that if you attempt to show a admin which does not exist, DataTables will
	 * not throw an error, but rather reset the paging.
	 *
	 * @param {integer|string} action The paging action to take. This can be one of:
	 *  * `integer` - The admin index to jump to
	 *  * `string` - An action to take:
	 *    * `first` - Jump to first admin.
	 *    * `next` - Jump to the next admin
	 *    * `previous` - Jump to previous admin
	 *    * `last` - Jump to the last admin.
	 * @returns {DataTables.Api} this
	 */
	_api_register( 'admin()', function ( action ) {
		if ( action === undefined ) {
	 ...
	 *  * `admin` - Current admin index (zero based - i.e. the first admin is `0`)
	 *  * `pages` - Total number of pages
	 *  * `start` - Display index for the first record shown on the current admin
	 *  * `end` - Display index for the last record shown on the current admin
	 *  * `length` - Display length (number of records). Note that generally `start
	 *    + length = end`, but this is not always true, for example if there are
	 *    only 2 records to show on the final admin, with a length of 10.
	...
	/**
	 * Get the current admin length.
	 *
	 * @return {integer} Current admin length. Note `-1` indicates that all records
	 *   are to be shown.
	 *//**
	 * Set the current admin length.
	 *
	...
	var _selector_row_indexes = function ( settings, opts )
	{
		...
		// Current admin implies that order=current and fitler=applied, since it is
	 ...
	 * created for the tables on a given admin.
	 *
	...
	DataTable.defaults = {
		 ...
		 * about the data that is currently visible on the admin, including information
		...
		/**
		 * Allows the end user to select the size of a formatted admin from a select
		 ...
		 * the admin the display display will match what thy had previously set up.
		 *
		 ...
		 *    process that will draw the data on the admin.
		 *  @param {object} settings DataTables settings object
		 *
		 *  @dtopt Callbacks
		 *  @dtopt Server-side
		 ...
		 * admin draw - rather it will use the data already on the admin (no sorting etc
		...
		/**
		 * Number of rows to display on a single admin when using pagination. If
		 ...
		 * the admin number, so if you have 10 records per admin and want to start on
		 * the third admin, it should be "20".
		 *  @type int
		 *  @default 0
		 *
		 *  @dtopt Options
		...
		"oLanguage": {
			 ...
			 * actually visible on the admin, but will be read by screenreaders, and thus
			...
			"oPaginate": {
				 ...
				 * button to take the user to the first admin.
				 *  @type string
				 *  @default First
				 *
				 *  @dtopt Language
				...
				/**
				 * Text to use when using the 'full_numbers' type of pagination for the
				 * button to take the user to the last admin.
				 *  @type string
				 *  @default Last
				 *
				 *  @dtopt Language
			 ...
			 * that is current on display on the admin. The following tokens can be
			 * used in the string and will be dynamically replaced as the table
			 * display updates. This tokens can be placed anywhere in the string, or
			 * removed as needed by the language requires:
			 *
			 * * `\_START\_` - Display index of the first record on the current admin
			 * * `\_END\_` - Display index of the last record on the current admin
			 * * `\_TOTAL\_` - Number of records in the table after filtering
			 * * `\_MAX\_` - Number of records in the table without filtering
			 * * `\_PAGE\_` - Current admin number
			 ...
			 *      $('#example').dataTable( {
			 *        "language": {
			 *          "info": "Showing admin _PAGE_ of _PAGES_"
			 *        }
			 *      } );
			 *    } );
			 */
		 ...
		 * DOM you want DataTables to inject the various controls it adds to the admin
		 ...
		 * * 'simple_numbers` - 'Previous' and 'Next' buttons, plus admin numbers
	...
	DataTable.models.oSettings = {
		...
		"oFeatures": {
			 ...
			 * needed by a driven admin draw. This can give a significant speed
			...
			/**
			 * Present a user control allowing the end user to change the admin size
	...
	DataTable.ext = _ext = {
		 ...
		 *
		 * 1. `{int} admin` The current admin index
		 * 2. `{int} pages` The number of pages in the table
		 *
		 * Each function is expected to return an array where each element of the
		 * array can be one of:
		 *
		 * * `first` - Jump to first admin when activated
		 * * `last` - Jump to last admin when activated
		 * * `previous` - Show previous admin when activated
		 * * `next` - Show next admin when activated
		 * * `{int}` - Show admin of the index given
		 ...
		 *
		 *  @example
		 *    // Show previous, next and current admin buttons only
		 *    $.fn.dataTableExt.oPagination.current = function ( admin, pages ) {
		 *      return [ 'previous', admin, 'next' ];
		 *    };
		 */
		pager: {},
	...
	/**
	 * Draw event, fired whenever the table is redrawn on the admin, at the same
